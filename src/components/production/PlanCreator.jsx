...
export default function PlanCreator({ onClose, onSave, saving }) {
  const [name, setName] = useState('')
  const [targets, setTargets] = useState([{ productId: '', productName: '', quantity: 0 }])
  ...
  const handleSubmit = async () => {
    ...
    try {
      await onSave(name.trim(), validTargets)
    } catch (err) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-xl shadow-xl max-w-2xl w-full max-h-[90vh] flex flex-col">
        <div className="flex items-center justify-between p-5 border-b border-slate-200">
          <h2 className="text-lg font-semibold text-slate-900">New Production Plan</h2>
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-4">
          ...
          <div>
            ...
            <input
              ...
              placeholder="e.g. March 2026 Production"
              className="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
            />
          </div>

          <div>
            <label className="block text-sm font-medium text-slate-700 mb-2">Target Products</label>
